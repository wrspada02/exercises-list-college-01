...
""" 9º) Uma grande emissora de televisão quer fazer uma enquete entre os seus telespectadores para
saber qual o melhor jogador após cada jogo. Para isto, faz-se necessário o desenvolvimento de um
programa, que será utilizado pelas telefonistas, para a computação dos votos. Sua equipe foi
contratada para desenvolver este programa, utilizando a linguagem de programação Python. Para
...
número inválido for digitado, o programa deve ignorá-lo, mostrando uma breve mensagem de aviso, e
...
4. O número do jogador escolhido como o melhor jogador da partida, juntamente com o número
...
função calculará o percentual e retornará o valor calculado. Abaixo segue uma tela de
exemplo. A disposição das informações deve ser o mais próxima possível ao exemplo. Os dados
são fictícios e podem mudar a cada execução do programa. Ao final, o programa deve ainda
gravar os dados referentes ao resultado da votação em um arquivo texto no disco, obedecendo
...
def exercise09():
  ...
  while player != 0:
    player = int(input('Número do jogador (0=fim): '))
    if player != 0:
  ...
  totalVotes = sum(players)

  print('Resultado da votação:')
  print(f'Foram computados {totalVotes} votos.')

  for i, playerVotes in enumerate(players):
    if playerVotes > 0:
      print(f'Jogador {i+1} {playerVotes} {round(calcPercentage(playerVotes, totalVotes), 2)}%')

  print(f'O melhor jogador foi o número {players.index(max(players)) + 1}, com {max(players)} votos, correspondendo a {round(calcPercentage(max(players), totalVotes), 2)}% do total de votos.')
